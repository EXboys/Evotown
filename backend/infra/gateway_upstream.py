...
def build_responses_upstream_call(
    ...
    """Build an upstream call for /responses.

    Codex requires the Responses wire API, but many managed upstreams only
    expose Chat Completions. Translate Responses → Chat and hit /chat/completions.
    """
    from infra.responses_bridge import responses_request_to_chat

    chat_body = responses_request_to_chat(body)
    target, headers, req = build_upstream_call(chat_body, effective_model)
    metadata = req.get("metadata") if isinstance(req.get("metadata"), dict) else {}
    req["metadata"] = {
        **metadata,
        "evotown_responses_via_chat": True,
        "evotown_client_protocol": "responses",
    }
    return target, headers, req
